...
dis(f"{OR}#8", -64, lambda: T3["seven_brane_charge"]["total_O7_charge_D7_units"], "comparison/third_computation_orientifold_results.json (T^2 fixed points = 4, Q(O7^-) = -4)",
    routes=1, note="DISAGREE. Sealed TadpoleCancellation.lean (v3.20.0) puts an O7 at each of the 16 fixed points of T^4/Z2 with Q = -4 D7 units (-64). The O7 planes of K3xT2/Z2 sit at the 4 fixed points of T^2/Z2 (TT lines 160-163, and the third computation's fixed-point count), giving -16. No unit conversion reconciles it: both sides use Q(O7^-) = -4 per plane; the plane COUNT differs (16 vs 4). The negative control in the third computation reproduces -64 exactly from the T^4 count. Post-seal LeanMaster (adc85e7, v3.21.0) changed this to numO7Planes = 4, total_O7_charge_is_minus_16.")
dis(f"{OR}#9", 64, lambda: T3["seven_brane_charge"]["total_D7_charge_D7_units"], "comparison/third_computation (16 D7 needed, unit charge)",
    routes=1, note="DISAGREE. Sealed: 32 D7 x charge 2 = 64. Required: 16 D7 x 1 = 16 (TT; third computation). Counting 16 D7 plus 16 images as 32 would need charge 1/2 each, not 2, so no counting convention reproduces 64 with Q(O7^-) = -4. Post-seal LeanMaster (adc85e7) changed to 16 D7 x 1 = 16.")
...
BLIND_ONLY = [
    ...
    "B: Hurwitz class numbers H(D) for D in 16..40 with reduced forms, whole-table Kronecker-Hurwitz validated n=1..10",
