"""
Track C (v2), item (2): K3 lattice = 3U + 2(-E8); Mukai lattice = K3 + U;
Gamma^{6,22} = 6U + 2(-E8).

Reused with only path changes from v1 (audit/k3t2_rigidity/lattices-duality/
02_k3_mukai_gamma.py, written blind). The (m,n)=(3,2) decomposition of K3
used HERE is a fixed, named construction whose Gram matrix is assembled and
then measured (rank/det/signature computed, not assumed) -- it is not the
rigidity claim. The rigidity claim about which (m,n) is forced is handled
separately and CHAINED to topology in 06_chained_rigidity.py; this script
only reports what 3U+2(-E8) etc. actually measure to be.

For each lattice: assemble the FULL explicit Gram matrix (block diagonal,
built from the same E8 Cartan matrix as item 1, and the hyperbolic plane U),
then compute rank, determinant, evenness, unimodularity, and signature (p,q)
...
assuming signature is additive over blocks):
  ...
  (B) exact characteristic-polynomial sign counting (sympy Sturm-based
      real_roots on QQ, with multiplicity), which is genuinely independent
      machinery.
Agreement of (A) and (B) is the "two different, same finding" check.

Run: /home/callensxavier_gmail_com/SocrateAI-Scientific-DualScaleSimulator/.venv-tda/bin/python \
     /mnt/disks/disk-socrateai-local-1/dualscale-wt-k3t2/audit/k3t2_rigidity_v2/C-lattices/02_k3_mukai_gamma.py
"""
import json
import sys
...
results["K3_3U_2negE8"] = analyze(
    ...
    {"signature": [3, 19], "det": 1, "even": True,
     "source": "standard K3 lattice fact, from memory, unverified prior to computation; "
               "NOT used to select (m,n) -- that selection is chained to topology "
               "independently in 06_chained_rigidity.py"},
)
...
out = {
    ...
    "method": "Explicit block-diagonal Gram matrices from U=[[0,1],[1,0]] and the E8 Cartan "
               ...
               "(congruence diagonalization over Q, and Sturm-sequence real-root counting "
               "on the exact characteristic polynomial) on the FULL matrix, not assumed additive.",
    "results": results,
}

out_path = HERE + "/02_k3_mukai_gamma_result.json"
