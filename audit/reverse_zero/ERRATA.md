...
These are corrections from the statistics and framing skeptics (runs wf_32d1ef7e-667 and wf_c11901b6-1ed). Numbers in script-generated JSON are left as committed, so reruns still reproduce them; this table gives the corrected reading. Quotations wrongly attributed to LeanMaster were removed in place from both the scripts and their JSON output: they were static strings, so no computed value changed. The hand-written Markdown files were corrected inline. Line numbers refer to commit b77d11a's successor. Where anything disagrees, `REPORT.md` is authoritative.

| Where | As committed | Correct reading |
|---|---|---|
| `E4-cosmic-web-tda/cosmic_web_tda.py` (the `interpretation` and `next_step_...` strings, now lines 240-266); `e4_cosmic_web_tda_report.json:58-59`; `e2_pta_and_tda_extension/IMPROVEMENT_PROPOSALS.md:82,98`; `IMPROVEMENT_PROPOSAL.md:18`. **Fixed in place.** The quote remains only in the untracked harness file `_prior_results_wf_32d1ef7e.json` (ground E4). | Quoted LeanMaster: "a TDA prediction would be ... This is frozen as Tier C", and "(H') ... to be frozen before comparison" as a LeanMaster E4 note | **No such text exists in LeanMaster.** A fixed-string grep over all tags finds nothing. LeanMaster STREAM8 says "Observables: none" (lines 112, 153, 374 at eb791e7). |
...
| `e2_pta_result.json:107`; `e2_pta_sensitivity_scan.py:171`. **Fixed in place.** | "LeanMaster Streams 6-8 confirm mu_sym and c4_pta_product are unchanged" | LeanMaster never mentions either parameter. "Untested" is this project's statement. |
| `E3-M0/m0_model.py:295`; `E3_RESULT.md:42` (fixed inline) | 9.00 called the "pre-registered rule" for M0 vs fitted LCDM | PRE_REGISTRATION.md:34-36 is only a threshold table. The M0 comparison was not pre-registered. |
| `E3_RESULT.md:42` (fixed inline) | "~0.1 sigma" | 0.86 sigma: chi2.sf(0.746, 1) = 0.388. |
...
| `E2_RESULT.md:80` (fixed inline) | slope 6.171 | 1.000 (`skeptic_statistics/check_e2_slope.json`). |
