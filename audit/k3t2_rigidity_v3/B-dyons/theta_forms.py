...
(q-truncated at QMAX, y within +-YMAX), and extract cB(D) = coefficients of B = phi_{0,1}
(v3: NO overall factor is applied here; the K3 normalisation Z = k*phi_{0,1}, k a DECLARED
INPUT, is applied downstream from inputs.json).

Run: cd audit/k3t2_rigidity_v3/B-dyons && <venv-python> theta_forms.py 40
     (committed run uses QMAX=40; a second run with QMAX=30 is used by theta_margin_check.py)
Writes theta_forms_cache_Q<QMAX>.json with all series + the cB(D) table + self-checks.
"""
import sys, json
...
if __name__ == "__main__":
    ...
    # cB(D): coefficient of phi_{0,1} at q^n y^l, D = 4n - l^2 (index-1 => depends on D only)
    cD = {}
    ...
    for D, vals in cD.items():
        if len(vals) == 1:
            cB[D] = frac(next(iter(vals)))
        else:
            conflict = True
    checks = {
        "A_index1_structure_holds": okA,
        "B_index1_structure_holds": okB,
        "cB_no_index1_conflict": (not conflict),
        "A_q0": {"y^1": frac(A.get((0, 1), Fr(0))), "y^0": frac(A.get((0, 0), Fr(0))),
                 "y^-1": frac(A.get((0, -1), Fr(0)))},
        "B_q0": {"y^1": frac(B.get((0, 1), Fr(0))), "y^0": frac(B.get((0, 0), Fr(0))),
                 "y^-1": frac(B.get((0, -1), Fr(0)))},
    }
    out = {
        "QMAX": QMAX, "checks": checks,
        "cB_table": cB,
    ...
    with open(here / f"theta_forms_cache_Q{QMAX}.json", "w") as f:
        json.dump(out, f)
