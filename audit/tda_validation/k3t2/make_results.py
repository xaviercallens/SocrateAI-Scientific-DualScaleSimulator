...
report = {
    ...
    "partB_summary": [
        ...
        "Positive control at the same dimension, same sampler, same projector embedding and same tau: the Fermat QUADRIC x^2+y^2+z^2+w^2=0 (S^2 x S^2, b=(1,0,2,0,1)). It is also not recovered, and it never left the NOISE regime at the sizes that fit: at N=2000 (3 seeds) beta_1 is still 1-9 and beta_2 is 3-17 near tau with no plateau; N=4000 exceeded memory (the quadric sample is denser than the K3 one at equal N, so it is more expensive and reaches a LOWER effective density than the K3 runs). That is a different failure mode from the K3 runs, which did leave the noise regime (beta_1 = 0 exactly, beta_2 flat at 27 across 3 seeds and across N=3000/tau=0.9). So the control neither reproduces nor rules out an over-count: the origin of the 27 is undetermined by these runs.",
