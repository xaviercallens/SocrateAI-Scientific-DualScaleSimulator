...
Three core controls:
  1. Positive: Scratch theorem using `rfl` → must be STANDARD
...
class TestLeanAxiomAudit:
    ...
    def test_positive_control_standard_theorem(self):
        """Positive control: theorem using rfl should be STANDARD."""
        scratch_content = """theorem test_one_plus_one : 1 + 1 = 2 := rfl
"""
        result = run_scratch_audit(scratch_content)
        ...
        print(f"✓ Positive control passed: {theorem_data['classification']}")

    def test_negative_control_custom_axiom(self):
        """Negative control: theorem depending on custom axiom should be CUSTOM_AXIOM."""
        scratch_content = """axiom my_custom_axiom : True

theorem uses_custom_axiom : True := my_custom_axiom
"""
        result = run_scratch_audit(scratch_content)
        # Status can be OK or FAIL; the important thing is the classification
        ...
        print(f"✓ Negative control passed: {theorem_data['classification']}")

    def test_malformed_lean_elaboration_error(self):
        """Malformed Lean in a theorem should result in elaboration error."""
...
"""
        ...
        print(f"✓ Malformed Lean test passed: {result['error'][:60]}")


def run_scratch_audit(lean_code: str) -> dict:
    """
    Write lean_code to a temporary scratch file and audit it.
    Returns the audit result dictionary.
    """
