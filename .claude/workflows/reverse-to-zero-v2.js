export const meta = {
  name: 'reverse-to-zero-v2',
  description: 'Reverse loop round 2: commit-frozen registration before any data is loaded, then the four ranked follow-ups from round 1 (CMB TDA with a spectrum-matched null, cosmic-web TDA with redshift-space mocks and a powered gate, NANOGrav 15-yr c4 bound, DESI DR2 + CMB + SN); verbatim-only LeanMaster quotes at HEAD; two skeptics',
  whenToUse: 'Next round after reverse-to-zero (2026-09-19 run, REPORT 13756dc, ERRATA b77d11a). Tests M0 (a hypothesis change, not a K3xT2 derivation) and tries to bound the untested c4_pta_product.',
  phases: [
...
const OUT = `${WT}/audit/reverse_zero_r2`
...
const ctx = `ROUND 1: ${R1}/REPORT.md, ${R1}/ERRATA.md, ${R1}/ledger_reverse_zero.json (read them; reuse the round-1 code, fixing the defects named there).
...
${ctx}
${regctx}
X4 - DESI DR2 BAO + COMPRESSED CMB + PANTHEON+. Reuse ${R1}/E1-desi-dr2/e1_desi_dr2.py and ${R1}/E3-M0/m0_model.py and unify them into one script with ONE SN cut (rule: 1580 primary, 1590 disclosed). Add the CMB distance priors from a published table: quote the paper, its table and its covariance verbatim, and check the numbers against a recomputation in CAMB at the paper's best fit. Report CPL vs Lambda with Omega_m free (2 dof); M0 vs fitted LCDM; and whether the published ~3.1 sigma is approached. For P1 this is informational (rule 10). Save under ${OUT}/X4-bao-cmb-sn/ and commit.`,
