...
__all__ = [
    ...
    "height",
    "is_invariant_under",
    "discriminant_group_order",
    "atkin_lehner_multiplier",
    "rho_AL_on_w",
]
...
def discriminant_group_order(N: int) -> int:
    """Order of the discriminant group of ``U + <2N>``, which is cyclic of order ``2N``.

    Stream 1, `ModularAction.lean` (commit `b34be5b`, 2026-09-21)::

        T_N = U + <2N> has T_N^v = Z e + Z f + (1/2N) Z w, so the discriminant group
        T_N^v / T_N is cyclic of order 2N, generated by w/2N.

    At this repository's level `N = 12` that is `Z/24` -- consistent with
    `det T_N = -2N = -24` (`audit/K3_SELECTION.md`).
    """
    return 2 * N


def rho_AL_on_w(N: int, a: int, b: int, c: int, d: int) -> np.ndarray:
    """``rho_AL`` applied to the generator ``w``, i.e. its third column.

    Mirrors `rhoAL_w_image` (Stream 1 `b34be5b`)::

        rhoAL N a b c d *v ![0,0,1] = ![2N(cd), -(2N(ab)), N a d + b c]

    Stream 1's note on why this matters, which is the part that is easy to assume
    rather than check: "The two `e`, `f` components below are visibly divisible by
    `2N`; that is **why** the action descends to the discriminant group at all."
    """
    return rho_AL(N, a, b, c, d) @ np.array([0, 0, 1], dtype=np.int64)


def atkin_lehner_multiplier(N: int, a: int, b: int, c: int, d: int) -> int:
    """The multiplier ``m = N a d + b c`` by which ``rho_AL`` acts on ``Z/2N``.

    Mirrors `rhoAL_disc_multiplier` and `rhoAL_disc_multiplier_congr`: under the
    normalization `N a d - b c = 1`, `m = 2 N a d - 1`, and `2N | (m + 1)` -- i.e.
    `m = -1 mod 2N`. At the Fricke element `(0, -1, 1, 0)`, `m = -1` exactly
    (`rhoAL_fricke_multiplier`): **the Fricke involution acts as `-1` on the whole
    discriminant group.**

    SCOPE, quoted from Stream 1 rather than paraphrased, because it is easy to
    overstate: "Stream 2 states the rule for a general Hall divisor `Q`, with
    ...
    The congruence holds only under the normalization: at `N = 7, a = d = 1,
    b = c = 2` (where `N a d - b c = 3`) the multiplier is `11`, not `-1`. Stream 1
    keeps that as a negative control and so does this module's test suite.
    """
    return N * a * d + b * c
